...
def odds_to_probabilities(home_odds: float, draw_odds: float, away_odds: float) -> tuple[float, float, float]:
...
def add_implied_probabilities(df: pd.DataFrame, bookmaker: str = "Market Avg") -> pd.DataFrame:
...
def odds_calibration(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if valid.empty:
        return pd.DataFrame()
    rows = []
    for result, label, col in [("H", "Home win", "ImpHome"), ("D", "Draw", "ImpDraw"), ("A", "Away win", "ImpAway")]:
        rows.append({
            "Outcome": label,
...
def similar_odds_matches(df: pd.DataFrame, home_prob: float, draw_prob: float, away_prob: float, tolerance: float = 0.04) -> pd.DataFrame:
